                    ...
                    <form action="{{ route('team-members.update', $teamMember) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="mb-4">
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $teamMember->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div class="mb-4">
                                <x-input-label for="role" :value="__('Role (e.g. K9, Medic, Tech)')" />
                                <x-text-input id="role" class="block mt-1 w-full" type="text" name="role" :value="old('role', $teamMember->role)" required />
                                <x-input-error :messages="$errors->get('role')" class="mt-2" />
                            </div>

                            <div class="mb-4">
                                <x-input-label for="phone" :value="__('Phone Number')" />
                                <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $teamMember->phone)" />
                            ...
                            <div class="mb-4">
                                ...
                                <select id="availability_status" name="availability_status" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="Available" @selected(old('availability_status', $teamMember->availability_status) == 'Available')>Available</option>
                                    <option value="On Mission" @selected(old('availability_status', $teamMember->availability_status) == 'On Mission')>On Mission</option>
                                    <option value="Off Duty" @selected(old('availability_status', $teamMember->availability_status) == 'Off Duty')>Off Duty</option>
                                </select>
